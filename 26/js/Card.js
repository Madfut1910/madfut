...
async function fetchLivePlayers() {
  ...
  try {
    ...
    const loadingScreen = document.getElementById('loading-screen') || document.getElementById('loading-text');
...
function buildFiltersFromPlayers() {
  ...
  if (typeof window._buildAndInjectEvoPlayers === 'function') {
    window._buildAndInjectEvoPlayers(() => {
      dataLoaded = true;
      filterAndSort();
    });
  } else {
    ...
    <div class="card-and-breakdown">
      <div class="card-menu-wrap">
        ...
        <div class="card-menu-dropdown">
          ...
          <div class="card-menu-item card-menu-scale" style="display:flex;align-items:center;gap:6px;pointer-events:none">
            <span style="font-size:11px;color:var(--muted)">Scale</span>
            ${[1,2,3,4].map(s => `<button class="scale-pick-btn${(window._downloadScale||3)===s?' active':''}" data-scale="${s}" style="pointer-events:auto;width:26px;height:22px;border-radius:4px;border:1px solid #ccc;background:${(window._downloadScale||3)===s?'var(--dark)':'#fff'};color:${(window._downloadScale||3)===s?'#fff':'var(--mid)'};font-size:10px;font-weight:700;cursor:pointer;font-family:'Roboto Condensed',sans-serif" onclick="event.stopPropagation();window._downloadScale=${s};this.closest('.card-menu-dropdown').querySelectorAll('.scale-pick-btn').forEach(b=>{const a=parseInt(b.dataset.scale)===${s};b.style.background=a?'var(--dark)':'#fff';b.style.color=a?'#fff':'var(--mid)';b.style.borderColor=a?'var(--dark)':'#ccc'})">${s}×</button>`).join('')}
          </div>
          <button class="card-menu-item" onclick="event.stopPropagation();openDevMenu(this)">Dev Menu</button>
        </div>
      </div>
