/* ============================================================
   26VotePromo.js  —  Combined votes + promos, sorted by date
   ============================================================ */

(function () {
  const VOTES_URL   = "https://firestore.googleapis.com/v1/projects/trivela-madfut/databases/(default)/documents/votes/";
  const PROMOS_URL  = "https://firestore.googleapis.com/v1/projects/trivela-madfut/databases/(default)/documents/promotions/";
  const LOCAL_JSON_URL = "https://firestore.googleapis.com/v1/projects/madfut-data/databases/(default)/documents/madfut/26/promo/";

  const EXCLUDED_VOTE_IDS  = new Set(["GbPl1Jyvs0KykC5R6Llq"]);
  const EXCLUDED_PROMO_IDS = new Set(["hiO0BIxs10uNjuXxA56N"]);
  ...
  /* ISO string → Date for sorting (fallback to epoch) */
  ...
  function buildVoteCard(v) {
    ...
    if (show && total > 0) {
      if (hasMiddle) { const max = Math.max(a,b,c); winA = a===max; winB = b===max; winC = c===max; }
      else { winA = a >= b; winB = b > a; }
    }

    const makeRow = (dotClass, name, p, cnt, winClass) => `
      <div class="vote-option-row${winClass ? ' '+winClass : ''}">
    ...
    const rowA = makeRow("vote-dot-a", v.nameLeft||"—",  pctA, cntA, winA?"vote-winner-a":"");
    const rowB = hasMiddle
      ? makeRow("vote-dot-b", v.nameMiddle||"—", pctB, cntB, winB?"vote-winner-b":"")
      : makeRow("vote-dot-b", v.nameRight||"—",  pctB, cntB, winB?"vote-winner-b":"");
    const rowC = hasMiddle
      ? makeRow("vote-dot-c", v.nameRight||"—",  pctC, cntC, winC?"vote-winner-c":"")
      : "";
  ...
  async function loadAll() {
    ...
    /* local JSON promos — merge if not already present */
    if (localRes.status === "fulfilled") {
      const raw = localRes.value;
      let localDocs = Array.isArray(raw) ? raw : raw.documents ? raw.documents : raw.fields ? [raw] : [];
      const existingPromoIds = new Set(items.filter(i => i._type==="promo").map(i => i.docId));
      localDocs
        .map(parsePromo)
        .filter(p => !existingPromoIds.has(p.docId))
        .forEach(p => items.push(p));
    }

    if (!items.length) {
      container.innerHTML = '<p class="votes-empty">Nothing found.</p>';
      return;
    }
    ...
    container.innerHTML = "";
    items.forEach(item => {
      container.appendChild(item._type === "vote" ? buildVoteCard(item) : buildPromoCard(item));
    });
  }
